// Local mock auth helper for the preview environment.
...
const authPaths = new Set(["/signin", "/signout", "/callback"]);
...
export function sites({ mockAuth = true } = {}): Plugin {
  ...
  return {
    ...
    configureServer(server) {
      ...
      server.middlewares.use((request, response, next) => {
        for (const name of Object.keys(request.headers)) {
          if (name.startsWith("site-authenticated-user-")) {
        ...
        if (!signIn && !signOut) {
          if (signInCookies.length === 1 && signInCookies[0] === "1") {
            setHeader(request, "site-authenticated-user-id", localUserId);
            setHeader(request, "site-authenticated-user-email", localEmail);
            setHeader(
              request,
              "site-authenticated-user-full-name",
              localFullName,
            );
            setHeader(
              request,
              "site-authenticated-user-full-name-encoding",
    ...
    async closeBundle() {
      ...
      const outputDirectory = resolve(root, "dist", ".site-config");
